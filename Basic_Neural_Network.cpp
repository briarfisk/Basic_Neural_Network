...
public:
	...
	bool OnUserUpdate(float fElapsedTime) override
	{
		...
		if (GetKey(olc::Key::X).bPressed) { std::cin >> tmp_X; Conman.X_Set(tmp_X); }
		if (GetKey(olc::Key::Y).bPressed) { std::cin >> tmp_Y; Conman.Y_Set(tmp_Y); }
		...
		if (Current_Menu_Method == 2)
		{
			if (Conman.is_CAN_Idle())
			{
				...
				if (Input == "-exit") { Input = "";  Current_Menu_Method = 8; return true; }
		...
		{
			if (Conman.is_CAN_Idle() && !flg_Has_Ran)
			{
				this->Clear(olc::BLACK); Conman.output_Nodes_GUI(this);
				std::string tmp_Input = "";
				Input = "";
			...
			else if (!(Conman.is_CAN_Idle()))
			{
				Conman.Build_Step();
			}
			else if (Conman.is_CAN_Idle() && flg_Has_Ran)
			{
				flg_Has_Ran = 0;
